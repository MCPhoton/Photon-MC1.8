...
/**
 * A lock-free queue based on a circular buffer using the "keep one slot open" technique. This
 * implementation is safe <b>only if there is ONE consumer and ONE producer</b>.
 *
 * @author TheElectronWill
 * @param <E> the type of elements held in this collection
 */
@SuppressWarnings("unchecked")
public class StrategicSPSCQueue<E> extends AbstractQueue<E>implements BlockingQueue<E> {
	...
	/**
	 * Tail of the queue. It's not thread-safe because only one producer modify it.
	 */
	 ...
	 */
	public StrategicSPSCQueue() {
		array = (E[]) new Object[16];
		moduloMask = 15;
		strategy = WaitStrategy.LOOP;
	}
	
	public StrategicSPSCQueue(final int capacity) {
	...
	public StrategicSPSCQueue(final int capacity, final WaitStrategy strategy) {
		int cap = nextPowerOfTwo(capacity + 1);
		array = (E[]) new Object[cap];
		moduloMask = cap - 1;
	...
	public boolean isEmpty() {
		// tail == head is NOT a reliable indicator !
	...
	public boolean isFull() {
		...
		return head == ((tail + 1) & moduloMask);
	}
	
	@Override
	public int size() {
		final int cTail = tail;
	...
	public boolean offer(E e) {
		...
		if (head == ((tail + 1) & moduloMask)) {// queue is full
			return false;
		}
		array[tail] = e;
		tail = (tail + 1) & moduloMask;// It's ok because only one thread updates the tail variable.
		return true;
	}
	
	@Override
	public E poll() {
		final E e = array[head];
